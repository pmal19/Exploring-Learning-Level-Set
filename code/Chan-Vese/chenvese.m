...
function [seg,phi] = chenvese(I,mask,num_iter,mu,method,targetClass,lambda)
...
switch lower(method)
    case {'chan','vector'}
        ...
        %-- Display settings
          ...
          % subplot(2,2,1); imagesc(I); title('Input Image');
        ...
        %-- Main loop
          for n=1:num_iter
              ...
              for i=1:layer
                  ...
                  c1_new = (c1 + lambda*targetClass)/(1+lambda);
                  
                  c2 = sum(sum(L.*(1-Heaviside(phi0))))/(length(outidx)+eps); % verage outside of Phi0

                  force_image=-(L-c1_new).^2+(L-c2).^2-lambda*(c1_new-targetClass)^2+force_image;
              ...
              if indicator % decide to stop or continue 
                  ...
                  phi = phi0;
                  subplot(1,2,2); imshow(seg); title('Feature Space Delineation');
    ...
    case 'multiphase'
        ...
        for n=1:num_iter
              ...
              if isempty(cc11)
                  ...
                  % c11 = mean(L(cc11));
                  c11 = (mean(L(cc11)) + lambda*targetClass)/(1+lambda);
              end
              ...
              f_image11=(((L-c11).^2 + lambda*(c11-targetClass)^2).*Heaviside(phi1).*Heaviside(phi2)) +f_image11;
